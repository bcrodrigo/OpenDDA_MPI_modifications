...
void broadcast_control_parameters(void){
   ...
   /* Calculate the data offsets fot the MPI_Type_create_struct datatype */
   ...
   MPI_Get_address(&control.number_of_materials,&displacements[13]);
   MPI_Get_address(control.wavelength_limits,&displacements[14]);
   MPI_Get_address(control.radius_limits,&displacements[15]);
   MPI_Get_address(control.euler_phi_limits,&displacements[16]);
   MPI_Get_address(control.euler_theta_limits,&displacements[17]);
   MPI_Get_address(control.euler_psi_limits,&displacements[18]);
   MPI_Get_address(control.theta_limits,&displacements[19]);
   MPI_Get_address(control.phi_limits,&displacements[20]);
   MPI_Get_address(&control.timing_enabled,&displacements[21]);
   MPI_Get_address(&control.DBLP,&displacements[22]);
   MPI_Get_address(&control.tensor_transpose,&displacements[23]);
   MPI_Get_address(&control.padded_transpose,&displacements[24]);
...
void broadcast_physical_parameters(void){
   ...
   /* Set the block lengths for the MPI_Type_create_struct datatype */
   blocklens[0]=1;
   blocklens[1]=1;
   blocklens[2]=1;
   blocklens[3]=1;
   ...
   /* Calculate the data offsets fot the MPI_Type_create_struct datatype */
   MPI_Get_address(&physical,&displacements[0]);
   MPI_Get_address(&physical.wavenumber,&displacements[1]);
   MPI_Get_address(&physical.radius_value,&displacements[2]);
   MPI_Get_address(&physical.target_dipole_spacing,&displacements[3]);
   MPI_Get_address(&physical.euler_phi_value,&displacements[4]);
   MPI_Get_address(&physical.euler_theta_value,&displacements[5]);
   MPI_Get_address(&physical.euler_psi_value,&displacements[6]);
   MPI_Get_address(physical.refractive_index_value,&displacements[7]);
   begin=displacements[0];
   for(i=0;i<count;i++){
      displacements[i]-=begin;
   }
   ...
   /* Create and commit the MPI Datatype */
   MPI_Type_create_struct(count,blocklens,displacements,types,&broadcast_physical_type);   
