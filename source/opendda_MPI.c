...
/* Allocate memory */
   ...
   if (parallel.myrank == 0) {
      // Open a file that will store the output from the efficiencies calculation
      q_output = fopen("Q_rawoutput.csv","w");
      fprintf(q_output,"wavelength,effective_radius,euler_theta,euler_psi,euler_phi,pol_state,Qext,Qabs,Qsca\n");


      // Open a file that will store the output from the fmatrix calculation
      fmatrix_output = fopen("fmatrix_rawoutput.csv","w");
      fprintf(fmatrix_output,"wavelength,effective_radius,euler_theta,euler_psi,euler_phi,pol_state,sca_phi,sca_theta,f_1p,f_2p\n");

      printf("\nOpen DDA (Scattering from a single particle) with %d processors\n",parallel.np);
   }
...
/* Create the FFTW DFT plans */
            ...
            for(p0=0;p0<euler_psi.list_length;p0++){
               ...
               for(f0=0;f0<euler_phi.list_length;f0++){
                  ...
                  for(polarisation_state=0;polarisation_state<polarisations;polarisation_state++){
                     ...
                     if (parallel.myrank == 0) {
                        printf("Wavelength: %+.*g Reff: %+.*g Orientation:(%+.*g,%+.*g,%+.*g) Polarization: %d\n",
                           DBLP,wavelength.value,
                           DBLP,radius.value,
                           DBLP,euler_theta.value,DBLP,euler_psi.value,DBLP,euler_phi.value,
                           polarisation_state);
                     } // end restrict to master
                     ...
                     /* Calculate the scattered field in the specified directions */
                     scattered_field_MPI();
   ...
   if(parallel.myrank==0){ /* Restrict to master */
      ...
      fclose(q_output); /* Close file with Q values*/
      fclose(fmatrix_output); /* Close file with fmatrix values*/
      printf("Simulation complete. All files closed\n");
   } /* End restrict to master */
